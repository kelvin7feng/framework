...
MySQLClient::MySQLClient(const std::string &host, const std::string &user, const std::string &password, const std::string &database, unsigned int port, const char *unixSocket, unsigned long clientFlag) {
	_host = host;
	_user = user;
	_password = password;
	_database = database;
...
MySQLClient::MySQLClient(const MySQLClient &db) {
	_host = db._host;
	_user = db._user;
...
void MySQLClient::SetInitialCommand(const std::string &initialCommand) {
	_initialCommand = initialCommand;
}

bool MySQLClient::Autocommit() const {
	return _autocommit;	
}

void MySQLClient::Autocommit(bool setvalue) {
	int ac = (setvalue) ? 1 : 0;
...
void MySQLClient::StartTransaction() {
...
void MySQLClient::UseDatabase(const string &dbName) {
...
int MySQLClient::Execute(const string &statement) {
...
void MySQLClient::LibraryInitialize(bool embedded) {
	_embedded = embedded;
	if (! embedded) {
...
void MySQLClient::ThreadInitialize() { 
	if (mysql_thread_init() != 0) {
		throw DatabaseException("Error in Database::ThreadInitialize", 0, "----", "Failure to initialize database for this thread");
	}
}

void MySQLClient::LibraryFinalize() {
	mysql_library_end();
}

void MySQLClient::ThreadFinalize() {
	mysql_thread_end();
}

LibraryInitializer::LibraryInitializer(bool embedded) {
	MySQLClient::LibraryInitialize(embedded);
}

LibraryInitializer::~LibraryInitializer() {
	MySQLClient::LibraryFinalize();
}

ThreadInitializer::ThreadInitializer() {
	MySQLClient::ThreadInitialize();
}

ThreadInitializer::~ThreadInitializer() {
	MySQLClient::ThreadFinalize();
}

bool MySQLClient::_embedded;
...
void MySQLClient::Source(const string &fileName) {
