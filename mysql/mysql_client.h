...
namespace MySQLWrap { 
	...
	public:
		MySQLClient(const std::string &host, const std::string& user, const std::string &password, const std::string &database, unsigned int port, const char *unixSocket, unsigned long clientFlags);
		MySQLClient();
		MySQLClient(const MySQLClient &db);
		virtual ~MySQLClient();
